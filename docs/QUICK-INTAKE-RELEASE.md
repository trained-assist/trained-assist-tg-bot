...
fail 6/8 cases; the repaired complete suite passes 197 tests.
...
intake/STT configuration is missing, the primary backend lacks the route, or primary auth/network fails. An online regional backend without the route also fails; an offline region is reported explicitly and uses the existing primary fallback in pickAgentUrl. Results
...
Additional local check against the real agent runQuickAnswer: the HH review request invokes the intent verifier; mocked NO returns null and mocked YES returns the review URL (2 verifier calls, no external API calls). The product-owner profile has no HH token, so its live review probe correctly returned answer:null; this is not evidence for a connected recruiter profile. No Telegram messages were injected.
