...
import { handleCommand, isQuietGroupForwardedCommand } from './handlers/commands.js';
...
async function dispatchInner(update, env) {
  ...
  // Admin group: user-mgmt commands + self-service agent commands (e.g. /get_webpass)
  ...
  if (String(chatId) === env.ADMIN_GROUP_ID) {
    ...
    } else if (isQuietGroupForwardedCommand(text)) {
      // Strip the bot mention so the agent sees a clean "/get_webpass".
