...
it('any logged-in profile can request the hidden restart command',async()=>{
 ...
 await handleCommand({chat:{id:123},from:{id:123},text:'/restart'},env);
 expect(calls.find(c=>c.url.endsWith('/maintenance')).body).toEqual({action:'request',initiator:'ordinary-engineer'});
