...
describe('durable restart outbox',()=>{
  it('persists a large attachment before ACK; reconstruction and retry retain the exact payload',async()=>{
    ...
    mockFetch(async (url,opts)=>{
      if(url.includes('telegram')) { expect(JSON.parse(opts.body).message_thread_id).toBe(42); return Response.json({ok:true}); }
    ...
    expect(sent[1]).toEqual(sent[0]); expect(sent[1].initiatedAt).toBe(1234); expect(sent[1].threadId).toBe(42); expect(sent[1].fileBase64).toBe(file);
